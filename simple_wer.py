...
def AverageWERs(hyps, refs, verbose=True, diagnosis=False):
    ...
    for idx, (hyp, ref) in enumerate(zip(hyps, refs)):
        ...
        if errs_i['sub'] != 0 or errs_i['ins'] != 0 or errs_i['del'] != 0:
            sentence_error += 1
            #print('error count: {}\tline {}'.format(error_cnt, idx + 1))
            #print(errs_i)
            # print(hyp.strip() + "\t" + ref.strip())
            #print(ref)
        if errs_i['ins'] + errs_i['sub'] + errs_i['del'] > 0:
            #print('error count: {}\tline {}'.format(error_cnt, idx + 1))
            #print(errs_i)
            #print(hyp, end='')
            #print(ref)
    ...
    if verbose:
        ...
        print(str_summary)
        print(str_details)
        print('SER: {:.2f}%'.format(1.0 * sentence_error / totals * 100))

    return total_errs, totalw, aligned_html_list


def main(argv):
    hyp = open(argv[1], 'r').readlines()
    ref = open(argv[2], 'r').readlines()
    ...
    errs, nref, aligned_html = AverageWERs(hyp, ref, verbose=True, diagnosis=diagnosis)
